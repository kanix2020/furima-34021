class OrderAddress
  ...
  with_options presence: true do
    ...
    validates :postcode, format: { with: /\A[0-9]+\z/, message: 'は半角数値でなければ登録できません。' }
    validates :prefecture_id
    validates :city
    validates :block
  ...
  def save
    ...
    # 住所情報を保存
    Address.create(building: building, postcode: postcode, city: city, prefecture_id: prefecture_id, block: block, phone_number: phone_number, order_id: order.id)
  end
